...
if __name__ == "__main__":
    ...
    def arrive_announce(id):
        arrive_path = "~/music/"+'arrive' + str(id) +".mp3"
        os.system('mplayer %s' %arrive_path)

    if(input == '1'):
        recognize_flag = True
        print("The first Step use None id:" + str(find_id))
        navi.goto(goals[0])  # go to NO.1 task information point
        print(goals[0])
        rospy.sleep(2)
        recognize_flag = True
        print("This is a test which the id is:"+str(find_id))
        rospy.sleep(2)
        navi.goto(goals[find_id])
        arrive_announce(id=find_id)
        ros.sleep(2)
        navi.goto(goals[9])  # goto NO.2 task information point
        rospy.sleep(1)
        recognize_flag = True
        navi.goto(goals[find_id])
        arrive_announce(find_id)
        ros.sleep(2)
        navi.goto(goals[10]) # go to NO.3 task information point
        ros.sleep(1)
        recognize_flag = True
        navi.goto(goals[find_id])
        arrive_announce(find_id)
        ros.sleep(2)
        navi.goto(goals[11]) # go to NO.4 task information point
        ros.sleep(1)
        navi.goto(goals[find_id])
        arrive_announce(find_id)
        ros.sleep(2)
